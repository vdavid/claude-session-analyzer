...
- **Colours are literals, not `var(…)`.** Canvas can't read custom properties, so charts take them from `theme.palette`,
  which re-reads the stylesheet when `prefers-color-scheme` flips. Never hardcode a hex here, texture ink included:
  `KindPie`'s decal reads `palette.chrome.decalInk`, the same `--csa-decal-ink` the CSS clock bars use.
- **A chart that doesn't need canvas doesn't belong here.** `ToolClockBars` and `BandBar` are HTML in `components/`:
  real text labels, real buttons, colours straight out of the stylesheet. Reach for ECharts when there's a scale, a
  zoom, or hundreds of marks.
...
- `ToolPie.svelte`: donut of tool calls, coloured by tool category rather than group. Slices of one category are
  adjacent and share a colour by design; the 2px surface ring, the legend, and the shared highlight tell them apart. Its
  tooltip names every clock it prints, because three of them arrive under one group's name.
