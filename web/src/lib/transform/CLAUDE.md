...
- **`toolBreakdown` counts calls, never seconds**, and orders by family for the same reason. A pie of tool seconds says
  ...
  each family one contiguous arc, which is the fixed adjacency the palette was validated against.
...
- `tools.ts`: `toolBreakdown`, the API's tool groups ordered for the tool donut, with the family arcs over them.
